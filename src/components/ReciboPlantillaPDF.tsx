...
const SingleReceipt = ({ recibo, detalles, alumno, copyName }: Props & { copyName: string }) => {
  ...
  return (
    <div className="w-full mb-1 border-t-2 border-dashed border-gray-400 pt-1 first:border-0 first:pt-0">
      <div className="border border-black bg-white">
        ...
        />
        <HeaderRow 
          label="ALUMNO (A) DEL" value={alumno?.grado_actual || ''} 
          label2="DE LA LICENCIATURA EN" value2={alumno?.licenciatura || ''} 
        />
        <div className="flex border-b border-black">
        ...
        <div className="flex border-b border-black bg-gray-200 font-bold text-[9px] text-center">
          <div className="w-[12%] p-1 border-r border-black flex items-center justify-center">CANTIDAD</div>
          <div className="w-[38%] p-1 border-r border-black flex items-center justify-center">CONCEPTO</div>
          <div className="w-[15%] p-1 border-r border-black flex items-center justify-center">COSTO UNITARIO</div>
          <div className="w-[15%] p-1 border-r border-black flex items-center justify-center">TOTAL</div>
          <div className="w-[10%] p-1 border-r border-black flex items-center justify-center">FORMA DE PAGO</div>
          <div className="w-[10%] p-1 flex items-center justify-center">BANCO</div>
        </div>

        {/* Details Rows */}
        {detalles.map((d, i) => (
          <div key={i} className="flex border-b border-black text-[9px]">
            <div className="w-[12%] py-[2px] border-r border-black flex items-center justify-center">{d.cantidad}</div>
            <div className="w-[38%] py-[2px] px-1 border-r border-black flex items-center truncate uppercase">{d.concepto}</div>
            <div className="w-[15%] py-[2px] border-r border-black flex items-center justify-center">${Number(d.costo_unitario).toFixed(2)}</div>
            <div className="w-[15%] py-[2px] border-r border-black flex items-center justify-center font-bold">${Number(d.subtotal).toFixed(2)}</div>
            <div className="w-[10%] py-[2px] border-r border-black flex items-center justify-center uppercase truncate">{i === 0 ? recibo.forma_pago : ''}</div>
            <div className="w-[10%] py-[2px] flex items-center justify-center uppercase truncate">{i === 0 ? recibo.banco : ''}</div>
          </div>
        ))}
        {Array.from({ length: blankRows }).map((_, i) => (
          <div key={`blank-${i}`} className="flex border-b border-black text-[9px] h-[19px]">
             <div className="w-[12%] border-r border-black"></div>
             <div className="w-[38%] border-r border-black"></div>
